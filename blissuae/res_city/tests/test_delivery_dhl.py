...
class TestDeliverysmsa(TransactionCase):

    def setUp(self):
        super(TestDeliverysmsa, self).setUp()
    ...
    @unittest.skip("smsa test disabled: We do not want to overload smsa with runbot's requests")
    def test_01_smsa_basic_us_domestic_flow(self):
        ...
        so_vals = {'partner_id': self.delta_pc.id,
                   'carrier_id': self.env.ref('delivery_smsa.delivery_carrier_smsa_dom').id,
                   'order_line': [(0, None, sol_vals)]}

        sale_order = SaleOrder.create(so_vals)
        sale_order.get_delivery_price()
        self.assertTrue(sale_order.delivery_rating_success, "smsa has not been able to rate this order (%s)" % sale_order.delivery_message)
        # smsa test server will return 0.0...
        # self.assertGreater(sale_order.delivery_price, 0.0, "smsa delivery cost for this SO has not been correctly estimated.")
        ...
        self.assertIsNot(picking.carrier_tracking_ref, False, "smsa did not return any tracking number")
        # self.assertGreater(picking.carrier_price, 0.0, "smsa carrying price is probably incorrect")
    ...
    @unittest.skip("smsa test disabled: We do not want to overload smsa with runbot's requests")
    def test_02_smsa_basic_international_flow(self):
        SaleOrder = self.env['sale.order']
        ...
        so_vals = {'partner_id': self.agrolait.id,
                   'carrier_id': self.env.ref('delivery_smsa.delivery_carrier_smsa_intl').id,
                   'order_line': [(0, None, sol_vals)]}
    ...
    @unittest.skip("smsa test disabled: We do not want to overload smsa with runbot's requests")
    def test_03_smsa_multipackage_international_flow(self):
        SaleOrder = self.env['sale.order']
