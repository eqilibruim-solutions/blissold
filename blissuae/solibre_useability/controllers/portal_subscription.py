...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        values['subscription_count'] = request.env['subscription.subscription'].search_count(self._get_subscription_domain(partner))
        return values
    ...
    def my_subscription(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, **kw):
        ...
        archive_groups = self._get_archive_groups('subscription.subscription', domain)
        ...
        return request.render("subscription.portal_my_subscriptions", values)
...
class sale_subscription(http.Controller):
    ...
    def subscription(self, account_id, uuid='', message='', message_class='', **kw):
        ...
        display_close = account.state == 'running'
        ...
        # if account.interval_type != 'weekly':
        ...
        dummy, action = request.env['ir.model.data'].get_object_reference('solibre_useability', 'action_subscription_form')
        values = {
            'account': account,
            ...
            'pricelist': account.doc_source.pricelist_id.sudo(),
        ...
        return request.render("subscription.subscription", values)
    ...
    def close_account(self, account_id, uuid=None, **kw):
        ...
        if kw.get('closing_text'):
            account.doc_source.message_post(_('Closing text : ') + kw.get('closing_text'))
        account.set_done()
            # account.date = datetime.date.today().strftime('%Y-%m-%d')
        return request.redirect('/my/home')


    @http.route(['/my/subscription/<int:account_id>/resume'], type='http', auth="public", website=True)
    def resume_account(self, account_id, uuid=None, **kw):
        account_res = request.env['subscription.subscription']
